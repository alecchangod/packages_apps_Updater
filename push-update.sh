...
updates_dir=/data/xtended_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have Xtended-VERSION-DATE-TYPE-* as format"
...
# Assume Xtended-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.xtended.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.xtended.updater/databases/updates.db" \
